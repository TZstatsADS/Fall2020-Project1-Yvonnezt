...
oid sha256:c44b827fa37a83366ceb80fb3bae5549de857f0a4b789529ee62f4bbf912f9e4
size 34502
=======
---
...
From the 2016 election, we came to realize that the polling data can be significantly deviated from the ground truth. When various media industry are using poll data to make statistical inference to predict that Hillary would won the election in 2016, rarely are they realize that the inference approach is limited. There are two reasons. First, the sample data may not be representative. As far as we know by prospect the 2016 election, we realize that most of Trump's voters are not quite active in polling investigation, which biases the inference result, and lead to the unexpected winning of Trump. Secondly, the inference method is strightforward and naive. Without looking into how attributes can synergize together and affect the voting, the inference result would be unreliable. 
...
Section 2 laod R packages and data and do data cleaning and pre-processing. Section 3 and 4 talk about the univariate and mutivariate analysis, for statistical method logistic regression model and polling method, respectively. Section 5 summarizes our observations and concludes the report. 
...
For data preprocessing, first, I choose 8 variables, which include Year, Turnout, Region, Income, Work, Education, Race and Gender, as demographic informations. Secondly, I choose variables that indicate the activities of voters related to the election such as meeting, activities, care, tv, money. In addition, I use model selection function to get the most usefull variables. Thirdly, I clean my data and remove missing value. I keep the election years only, and removed all the missing value in the selected data. Since the missing values in some variable in my dataset is Missing Completely at Random and the number of missing values is not very high, I can drop missing entries. lastly, I deal with variable Turnout: change it to be 1 (voted) and 0 (did not vote).

Description of variables:
`meeting` - Respondent attend political meetings/rallies during the campaign
`activities` - count of campaign participation activities 6-category  (1 lowest level of participation, 2,..., 6 Highes level of participation)
`care` - Respondent care which party wins presidential election
`tv` - Watch TV Programs about the Election Campagins
`money` - Give money to individual candidate during the campaigns

Some explainations about missing value:
The survey might exists some bias. For instance Survey question bias and Non-response bias. 

For the Survey question bias, it might occurs in the variable work and activities variables. The questions are hard to understand and making it difficult for customers to answer honestly. For example, the question in `activities` variable is 
`count of campaign participation activities 6-category  (1 lowest level of participation, 2,..., 6 Highes level of participation)` When I need to choose level 3?  If I attend the meeting 3 times a year or 10 times a year?

For the Non-response bias, it is a type of bias that happens when some people fail to respond to a survey. People may refuse to answer, or lack the time or inclination to answer.For example, people may refuse to answer `money` variable. The responders might not want other people know they didn't donate or give money to individual candidate, so they refuse to answer question.    
...
### Model selection 
A good model is the one that has minimum AIC among all the poptential models. A lower AIC value indicates a better fit of the data.
By using stepwise model selection, the finalmodel performance has the minimus AIC. For example, Model 1 uses only one deomgraphic variables 'Year' in the training dataset and obtains AIC 7411.575, whereas the final model obtains AIC 6060.16.
...
```
  ...
  + The estimate of `North Central (IL, IN, IA, KS, MI, MN, MO, NE, ND` (Region2 North Central) is positive value 0.18 that means all else being equal, the citizens who in (Region2 North Central) were more likely to vote comparing to the `Northeast (CT, ME, MA, NH, NJ, NY, PA, RI, VT)` (Region 1 Northeast) citizens. The estimate of `South (AL, AR, DE, D.C., FL, GA, KY, LA, MD, MS, NC` (Region 3 South) and `West (AK, AZ, CA, CO, HI, ID, MT, NV, NM, OR, UT, WA` (Region 4 West) are all negative values , - 0.37 and -0.11 respectively. Comparing to the (Region 1 Northeast) participants with all else being equal, the participants in (Region 3 South) and (Region 4 West) are less likely to vote. 
  + In variable Region, the Region 3 (South) have p-value less than 0.05 (significant threshold)
...
```
  + The coefficients of Income 2,3,4 and 5 are all positive means comparing to the partipants' income in `0 to 16 percentile` (Income1) others are more likely to vote. Meanwhile, the estimate of `17 to 33 percentile` (Income2) is 0.44 and the estimate of 68 to `95 percentile` (Income4) is 1.03. We can conclude that the effect of having Income in range `17 to 33 percentile` (Income2) is approximately twice as big in terms of leading to vote as having Income in range `68 to 95 percentile` (Income4).
  + All income levels are significant.
  + **`Income in range 68 to 95 percentile` (Income4) is the most likely income levels to vote; `0 to 16 percentile` (Income1) is the least likely income group to vote in the election.**
  
##### Variable: Work
```{r, echo=FALSE}
table(as.character(anes_use$Work))
```
  + Except the Work level2, The coefficent of the rest work groups are all negative. Comparing to the respondents in Work level1 and level2, the respondents in other Work groups are less likely to vote with all other things being equal. 
  + Work level3 and Work level4 have signifiance value.
  + **`Clerical and sales workers` (Work level2) is the most likely work group to vote while `Laborers, except farm` (Work level4) is the least likely work group to vote in the election.**

##### Variable: Education
```{r, echo=FALSE}
table(as.character(anes_use$Education))
```
  + Except the `High school (12 grades or fewer, incl. non-college` (Education group 2), all other groups have positive estimate. In contrast to the Education group `Grade school or less (0-8 grades)` (Education group 1), the participants with `Some college (13 grades or more but no degree` (Education group 3) and `College or advanced degree (no cases 1948)` (Education group 4) are more likely to vote with other things equal. Meanwhile, the effect of having Education group 4 [0.104] is approximately three time as big in terms of leading to vote as having Education group 3 [coeffection: 0.34]. In contrary, the partcipant only finish `High school (12 grades or fewer, incl. non-college` (Education group 2) less likely to vote in the election.
  + Education group 3 and Education group 4 have significant value.
  + **`College or advanced degree (no cases 1948)` (Education group 4) is the most likely education group to vote; `High school (12 grades or fewer, incl.non-college` (Education group 2) is the the least likely education group to vote in the election.**

##### Variable: Race
```{r, echo=FALSE}
table(as.character(anes_use$Race))
```
  + Besides the `Black non-Hispanic (1948-2012)` (Race group 2) and `White non-Hispanic (1948-2012)` (Race group 1), the rest race groups have negative coeffecient value means that the participants in other race groups are less likely to vote with others equal. Since Race group 2 have positive coefficient, we can conduce that the Race group 2 are more likely to vote.
  ...
  + **`Black non-Hispanic (1948-2012)` (Race group 2) is the most likely race group to vote; `American Indian or Alaska Native non-Hispanic (1966-2012)`(Race group 4) is the least likely race group to vote.**
  
##### Variable: Gender 
```{r, echo=FALSE}
table(as.character(anes_use$Gender))
```
  + The estimate of `Gender Female` has positive values that indicates Female more likely to vote in the election contract with `Gender Male`.
...
```
  + The estimate of `meeting Yes` is positive value. It shows that the respondents attend political meeting more likely to vote than the respondents never attend political meeting `meeting No`. 
...
```
  + `Care a good deal` has a positive coefficient value means that the respondents who care which party wins presidential election more likely to vote than the respondents who don't care very much `Don't care very much or DK, pro-con, depends`
  + `Care a good deal` has significant p-value.
  + **The respondent who care which party wins presidential election `Care a good deal` more likely to vote than the respondent who don't care very much `Don't care very much or DK, pro-con, depends`.**
...
```
  + `money Yes` has a positive coefficient value means that the respondents who give money to individual candidate during the campaigns are more likely to vote than the respondents who didn't give money `money No`. 
  + Not significant 
  + **The respondents who give money to individual candidate during the campaigns are more likely to vote than the respondents who didn't give money.**
...
$Sensitivity = \frac{number\ of \ Actual\ 1′s\ and\ Predicted\ as\ 1′s}{number\ of\ Actual\ 1′s} = 0.9227092$

Specificity (or False Positive Rate) is the percentage of 0’s (actuals) correctly predicted. 

$Sensitivity = \frac{number\ of \ Actual\ 1′s\ and\ Predicted\ as\ 1′s}{number\ of\ Actual\ 1′s} = 0.3011765$

The above numbers are calculated on the validation sample that was not used for training the model. So, a truth detection rate of 30% on test data is good.


##### By using the logist model, we can conclude  
**The most active group**:  Region: `North Central (IL, IN, IA, KS, MI, MN, MO, NE, ND` /n
                            Income:`68 to 95 percentile`/n
                            Work: `Clerical and sales workers`/n
                            Education: `College or advanced degree (no cases 1948)`/n
                            Race: `Black non-Hispanic (1948-2012)`/n
                            Gender: `Female`/n
                            meeting: `meeting Yes`/n
                            activities: `Activities level 4`/n
                            care: `Care a good deal`/n
                            tv: `Yes, watched TV program(s) about the campaign`/n
                            money:`money Yes` /n

##### By using the logist model, we can conclude the least active group 
**The least active group**: Region: `South (AL, AR, DE, D.C., FL, GA, KY, LA, MD, MS, NC` /n
                            Income: `0 to 16 percentile`/n
                            Work: `Laborers, except farm`/n
                            Education: `High school (12 grades or fewer, incl.non-college`/n
                            Race: `American Indian or Alaska Native non-Hispanic (1966-2012)`/n
                            Gender: `Male`/n
                            meeting: `meeting No`/n
                            activities: `Activities level 1`/n
                            care: `Don't care very much or DK, pro-con, depends`/n
                            tv: `No, didn't watch TV programs about the campaign`/n
                            money:`money No` /n

# Section 4: Polling Method

Besides statistical modeling, we also can use traditional polling method on ANES data to draw the inference on which group of people have high probability to vote. 

The first step: analyze the relationship between one single independent variable and dependent variable 
The second step: analyze multile variables at the same time
...
From the above analysis, after group by all the variables, we obtain 2614 subgroups that has voting propability equal to 1, which makes no sense. However, the reason is that such subgroups have only severaal data points, which measn the data size is not enough for reliable statistical inference. In the population data, the same subgroup selected by the same condition have far more voters comparing to the subgroup in our sample data, and the statistics of the sample is an unreliable estimate of the population. We can never conclude that as 2 which is all of the group of people in our sample voted in the election, so all people match the same condition in the population would always vote, without exemptions. It is clear that the result from polling is not robust and unreliable. 
